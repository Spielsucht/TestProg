...
namespace SpheroNET
{
    public static class SpheroCommandPacketFactory
    {
        ...
        public static SpheroCommandPacket Move(UInt16 heading, float speed)
        {
            byte _speed = (byte)(speed * 255f);
            byte msb = (byte)((heading & 0xFF00) >> 8);
            byte lsb = (byte)(heading & 0x00FF);
            return new SpheroCommandPacket(0x02, 0x30, 0x01, new byte[] { _speed, msb, lsb, 1 });
        }

        public static SpheroCommandPacket SetBackLED(double intensivity)
        {
            byte state = (byte)(intensivity * 255f);
            return new SpheroCommandPacket(0x02, 0x21, 0x01, new byte[] { state });
        }
